...
rewarding and fun was to have your tools tailormade specifically for your taste
and needs, and the next stop had to be my editor. [Neovim](https://neovim.io/)
was the cool kid on the block, without me realizing back then why, but I had to
give it a try.
...
from scratch a configuration using AI tooling, but even in this case, you need
...
which is my overall aisthetic and taste. Also because it was guiding you to
...
and ended up supporting all the languages I was using. To my surpise back then,
...
simple text editing over ssh, I started to undertand why Neovim was the cool kid
of the block. Neovim is a modern fork of Vim that brings Lua as the scripting
interface for configuration, built-in LSP support, and an extensible
architecture that makes it a joy to customize. You will never appreciate Lua,
unless you write vim script. Also, when compared to traditional IDEs, Neovim is
lightweight, runs in the terminal, and can be tailored to fit exactly how you
work. The "Edit Text at Speed of Though"

## General Structure of a Neovim Config

If you are thinking to build your onwn config, here's a chart I wish I had when
starting:

```mermaid!
flowchart TD
    A[init.lua] --> B[Plugin Manager<br/>lazy.nvim]
    A --> C[Core Options<br/>options.lua]
...
## Overview of the main moden Neovim plugins
...
One thing worth noting is the Poetry integration for Python projects. The
configuration automatically detects Poetry environments and uses the correct
`pylint` binary, which saves a lot of headaches when switching between projects.
