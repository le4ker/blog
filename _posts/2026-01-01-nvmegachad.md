...
kid of the block. Neovim is a modern fork of Vim that brings Lua as the
scripting interface for configuration, built-in LSP support, and an extensible
...
work. The "Edit Text at Speed of Though" which you can't comprehend unless you
experience it.
...
a few months core plugins were being replaced by new ones so I was playing cat
and mouse for a while, which some times was fun, other times it wasn't. Since
then the ecosystem has stabilized for about a year, so here are the core
pluggins and a sample set up.
